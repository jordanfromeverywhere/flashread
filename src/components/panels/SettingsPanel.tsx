...
import type { FontKey, Theme } from '../../lib/theme'
import { curateVoices, hasQualityVoice } from '../../lib/voices'
import { KOKORO_VOICES } from '../../lib/neuralTts'
import { cloudReady, loadCloud, saveCloud } from '../../lib/cloudTts'
...
const fieldStyle = (t: Theme): CSSProperties => ({
  width: '100%',
  boxSizing: 'border-box',
  font: '400 16px/1.3 ui-monospace,Menlo,monospace',
  padding: '10px 12px',
  borderRadius: 10,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <div ref={voiceRef} style={{ scrollMarginTop: 8 }}>
        ...
        {/* Where the neural voice is generated: this phone, or your own box. */}
        {s.neuralOn && (
          <div style={{ marginTop: 12 }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              <button onClick={() => s.cloudOn && a.setCloud(false)} style={pillStyle(t, !s.cloudOn)}>
                On this phone
              </button>
              <button
                onClick={() => !s.cloudOn && cloudReady() && a.setCloud(true)}
                style={{ ...pillStyle(t, s.cloudOn), opacity: cloudReady() ? 1 : 0.45 }}
              >
                My server
              </button>
            </div>
            <input
              defaultValue={loadCloud().url}
              onChange={(e) => saveCloud({ ...loadCloud(), url: e.target.value })}
            ...
            />
            <input
              defaultValue={loadCloud().key}
              onChange={(e) => saveCloud({ ...loadCloud(), key: e.target.value })}
              placeholder="Key (optional — must match FLASHREAD_KEY)"
              type="password"
              autoCapitalize="off"
              autoCorrect="off"
              spellCheck={false}
        ...
        <div style={{ font: '400 11px/1.5 sans-serif', color: t.sub, marginTop: 8 }}>
          {s.neuralOn ? (
            <>
              {s.cloudOn ? (
                <>
                  Generated by the server above and streamed a sentence at a time, so playback starts in about a second
                  and there is nothing to download. What you are reading is sent to that server — and nowhere else — as
                  it is spoken. See <span style={{ color: t.text }}>server/README.md</span> to deploy one.
                </>
              ) : (
                <>
                  A far more natural voice, generated locally — nothing is uploaded, and it works offline after a
                  one-time ~80&nbsp;MB download. On iPhone and iPad it runs on the CPU, which is slow: expect a long
                  wait before the first sentence, and use <span style={{ color: t.text }}>My server</span> above if you
                  have one.
                </>
              )}{' '}
              Turn on <span style={{ color: t.text }}>Audio</span> in the reader to listen.
            </>
          ) : (
            <>
              Voices come from your device, generated on-device — nothing is uploaded. Voices marked ✦ are the highest
              quality your browser will hand over. On iPhone and iPad that list is the basic set: Safari does not expose
              the “Enhanced” or Siri voices to web apps even once you have downloaded them, so{' '}
              <span style={{ color: t.text }}>Neural</span> is the only route to better narration there. On a Mac or PC,
              downloading a higher-quality system voice does show up here.
            </>
          )}
        </div>
      </div>
